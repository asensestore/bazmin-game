...
// District color tint modifiers
const DTINT={patriki:'#184818',ramenki:'#182848',mytishi:'#241810',krasnogorsk:'#2a1e08'};
...
function playCard(idx){
  ...
  if(b.usedCards.has(card.id)&&FREE_CARDS.includes(card.id)){
    // Free cards recharge
  } else if(b.usedCards.has(card.id)){
  ...
  const attack=Math.round(card.power*mult*(1+G.stats.charisma*.05));
  const defense=dem.power;
  const net=attack-defense;
  if(net>0){
    b.girlHP-=net;b.score+=net;
    const pct=mult>=1.8?'🎯 Отличный ответ!':mult>=1.4?'👍 Хороший ответ':' Нейтрально';
    document.getElementById('bLog').textContent=pct+' +'+net+' к обаянию';
  } else {
    b.heroHP+=net; // net is negative so subtract from hero
    document.getElementById('bLog').textContent='😬 Не то... -'+Math.abs(net)+' обаяния';
  }
  b.usedCards.add(card.id);
  if(!FREE_CARDS.includes(card.id))b.usedCards.add(card.id+'_perm');
  b.round++;
  haptic('light');
  setTimeout(()=>{
    if(b.round>=5||b.heroHP<=0||b.girlHP<=0)endBattle();
    else{updateBattleHUD();showNextDemand();}
  },600);
}
...
function render(){
  ...
  for(let r=0;r<ROWS;r++){
    for(let c2=0;c2<COLS;c2++){
      ...
      const baseCol=HTC[t]||'#16182e';
      let fillCol=baseCol;
      if(t===HT.PLAIN||t===HT.ROAD||t===HT.MKAD){
        // Tint by district
        fillCol=distTintHex(baseCol,dist);
      }
      // Draw hex
      drawHex(ctx,sx,sy,HSZ-1,inFog?'#0a0a14':fillCol,inFog?'#111118':'#ffffff14',1);
      if(!inFog){
        // Object emoji
        const obj=OBJ_HEXES[k];
        if(obj){
          ctx.font='18px serif';ctx.textAlign='center';ctx.textBaseline='middle';
          ctx.globalAlpha=.9;ctx.fillText(obj.emoji,sx,sy-4);ctx.globalAlpha=1;
        } else if(t===HT.PARK||t===HT.FOREST){
          ctx.font='14px serif';ctx.textAlign='center';ctx.textBaseline='middle';
          ctx.fillText((t===HT.PARK)?'🌳':'🌲',sx,sy);
        }
        // Grid lines subtle
        drawHex(ctx,sx,sy,HSZ-1,null,'#ffffff08',1);
      } else {
        // Fog of war gradient
        drawHex(ctx,sx,sy,HSZ-1,'#060610','#111',1);
      }
    }
  }
...
function distTintHex(base,dist){
  // Simple: return a slightly warmer/cooler color per district
  const tints={patriki:[0,12,0],krasnogorsk:[12,8,0],mytishi:[8,2,-4],ramenki:[0,4,12]};
  const t=tints[dist]||[0,0,0];
...
function renderMinimap(){
  ...
  for(let r=0;r<ROWS;r+=2){
    for(let c2=0;c2<COLS;c2+=2){
      ...
      const k=hexKey(c2,r);
      const inFog=!G.fog.has(k);
      if(inFog){mctx.fillStyle='#0d0d18';}
      else{
        const dist=getDistrict(c2,r);
        const cols={patriki:'#182820',krasnogorsk:'#282010',mytishi:'#202018',ramenki:'#181828'};
        mctx.fillStyle=t===HT.ROAD||t===HT.MKAD?'#2a2a38':t===HT.PARK||t===HT.FOREST?'#0a2a0a':OBJ_HEXES[k]?'#3a2a5a':(cols[dist]||'#181828');
      }
