...
function updateHUD(){
  ...
  const dayPhaseLabel=getDayAmbient?getDayAmbient().label:'';
  document.getElementById('turnLabel').textContent='Ур.'+G.level+' ✨'+(G.xp||0)+'/'+xpNeed+' · '+dayPhaseLabel+' День '+G.day;
  const mv=G.moves;
...
function render(){
  ...
  {const wx=getWeather();
  if(wx.id==='rainy'||wx.id==='snowy'){
    const count=wx.id==='snowy'?2:3;
    for(let i=0;i<count;i++){
      if(Math.random()<.3){
        const rx=Math.random()*W;
        particles.push({
          x:rx,y:-10,vx:wx.id==='snowy'?(Math.random()-.5)*.5:0.5+Math.random(),
          vy:wx.id==='snowy'?0.5+Math.random()*.5:2+Math.random()*2,
          emoji:wx.id==='snowy'?'❄':'💧',size:8+Math.random()*4,
          life:60+Math.random()*40,maxLife:100,_env:true
        });
      }
    }
  }}
