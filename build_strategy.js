...
<div id="apt" style="display:none">
  ...
  <div id="aptGrid"></div>
  <div style="font-size:11px;font-weight:700;color:#4fc3f7;margin-top:8px">🃏 Карточки (<span id="cardCountLbl">0</span>)</div>
  <div id="cardList" style="display:flex;flex-wrap:wrap;gap:6px;margin-top:4px"></div>
...
// ── Ambient music ─────────────────────────────────────────────────────────────
let bgMusic={gain:null,on:false};
// Chord progression: Am - F - C - G (A minor romantic)
const BG_CHORDS=[
  [220,261,330],[174,220,261],[130,164,196],[196,246,294]
];
let bgChordIdx=0,bgNoteTimer=null;
function startBGMusic(){
  if(bgMusic.on)return;
  const ac=getAC();if(!ac)return;
  bgMusic.on=true;
  const master=ac.createGain();master.gain.setValueAtTime(0,ac.currentTime);
  master.gain.linearRampToValueAtTime(0.03,ac.currentTime+3);
  master.connect(ac.destination);bgMusic.gain=master;
  function playChord(){
    if(!bgMusic.on||!bgMusic.gain)return;
    const chord=BG_CHORDS[bgChordIdx%BG_CHORDS.length];bgChordIdx++;
    const now2=ac.currentTime;const dur=3.5+Math.random();
    chord.forEach((freq,i)=>{
      try{
        const o=ac.createOscillator();const g2=ac.createGain();
        o.connect(g2);g2.connect(master);
        o.type='sine';o.frequency.value=freq;
        g2.gain.setValueAtTime(0,now2+i*.05);
        g2.gain.linearRampToValueAtTime(.4,now2+i*.05+.3);
        g2.gain.linearRampToValueAtTime(0,now2+dur);
        o.start(now2+i*.05);o.stop(now2+dur+.1);
      }catch(e){}
    });
    bgNoteTimer=setTimeout(playChord,(dur-0.5)*1000);
  }
  bgNoteTimer=setTimeout(playChord,800);
}
...
function endBattle(){
  ...
  if(win||b.round>=5&&b.heroHP>0){
    ...
    // Post-victory girl response based on personality and score
    const winResponses={
      romantic:['Это было волшебно... 💕','Ты меня покорил 🌹','Я так рада что мы встретились!'],
      intellectual:['Ты оказался интереснее чем я думала 📚','Мне понравился наш разговор!','Неожиданно умно. Зачёт.'],
      adventurous:['Вот это зажгли! Давай ещё! 🏃','Ты не скучный — это уже много!','Мне с тобой весело!'],
      materialistic:['Ладно... ты знаешь как угодить 💎','Неплохо для начала!','Продолжай в том же духе.'],
      caring:['Ты такой внимательный... 🌸','Я чувствовала твою заботу!','Ты настоящий.'],
      status:['Окей, ты прошёл проверку 👑','Мои подписчики одобрят.','Статусно получилось!'],
      trust:['Я начинаю тебе доверять 🤝','Ты честный. Это редкость.','Хорошее начало.'],
    };
    const pResponses=winResponses[b.girl.personality]||['Было приятно!'];
    const perfTxt=b.score>=80?pResponses[0]:b.score>=40?pResponses[1]:(pResponses[2]||pResponses[0]);
    document.getElementById('bResultTitle').innerHTML='<span style="color:#69f0ae">💕 '+perfTxt+'</span>';
...
const ACHIEVEMENTS=[
  ...
  {id:'veteran',name:'Ветеран',emoji:'🎖',check:()=>(G.day||1)>=20},
  {id:'collector',name:'Коллекционер',emoji:'🃏',check:()=>[...new Set(G.cards||[])].length>=8},
  {id:'valentines',name:'День Влюблённых',emoji:'💝',check:()=>G.storyFlags&&G.storyFlags.valentineDone},
];
...
function openApt(){
  ...
  const ccLbl=document.getElementById('cardCountLbl');
  if(cl){
    cl.innerHTML='';
    const uniqueCards=[...new Set(G.cards)];
    if(ccLbl)ccLbl.textContent=uniqueCards.length;
    uniqueCards.forEach(id=>{
      const card=ALL_CARDS[id];if(!card)return;
      const typeColors={attention:'#4fc3f7',mood:'#ffb74d',affection:'#f48fb1',romance:'#e91e63',luxury:'#ffd740',status:'#9c27b0',experience:'#69f0ae',generic:'#aaa',sweet:'#ff80ab',trust:'#81c784'};
      const tc=typeColors[card.type]||'#aaa';
      const span=document.createElement('div');
      span.style.cssText=\`display:flex;flex-direction:column;align-items:center;background:rgba(15,15,35,.9);border:1px solid \${tc}44;border-radius:8px;padding:6px 8px;min-width:50px;font-size:9px;text-align:center\`;
      span.innerHTML=\`<span style="font-size:18px">\${card.emoji}</span><span style="color:\${tc};font-size:8px">\${card.name}</span><span style="color:#888;font-size:7px">\${card.power}⚡</span>\`;
      cl.appendChild(span);
    });
  }
